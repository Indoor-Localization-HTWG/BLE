...
#define AOA_QUEUE_SIZE 8
K_MSGQ_DEFINE(aoa_msgq, sizeof(double), AOA_QUEUE_SIZE, 4); // 4-byte alignment
...
const static uint8_t ant_patterns[] = {0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8,
...
static void cte_recv_cb(struct bt_le_per_adv_sync *sync,
						struct bt_df_per_adv_sync_iq_samples_report const *report)
{
	double angle = calculate_aoa(report, 4);

	// Get the beacon address from the sync object
	struct bt_le_per_adv_sync_info info;
	bt_le_per_adv_sync_get_info(sync, &info);

	// Find the matching beacon
	for (int i = 0; i < NUM_BEACONS; i++)
	{
		if (bt_addr_le_cmp(&info.addr, &beacons[i].addr) == 0)
		{
			beacons[i].aoa_samples[beacons[i].write_idx] = angle;
			beacons[i].write_idx = (beacons[i].write_idx + 1) % MAX_AOA_SAMPLES;
			if (beacons[i].sample_count < MAX_AOA_SAMPLES)
			{
				beacons[i].sample_count++;
			}
			break;
		}
	}
	// Try to put the angle in the queue (do not block if full)
	int ret = k_msgq_put(&aoa_msgq, &angle, K_NO_WAIT);
	if (ret != 0)
	{
		printk("AoA queue full, dropping value\n");
	}
	printk("Estimated AoA: %.2f degrees\n", angle);
	...
	// Use first 4 antennas (first row) for demonstration
	if (report->sample_count >= 4)
	{
		double angle = calculate_aoa(report, 4);
		printk("Estimated AoA (first row): %.2f degrees\n", angle);
	}
...
int main(void)
{
	...
	do
	{
		...
		for (int i = 0; i < NUM_BEACONS; i++)
		{
			...
			for (int j = 0; j < beacons[i].sample_count; j++)
			{
				int real_idx = (idx + j) % MAX_AOA_SAMPLES;
				printk("  %.2f\n", beacons[i].aoa_samples[real_idx]);
			}
		}
		estimate_position();
	} while (true);
