...
/* Initialize Kalman filter with a neutral angle, cov=1.0 */
kalman1d_t kf;
static void cte_recv_cb(struct bt_le_per_adv_sync *sync,
			struct bt_df_per_adv_sync_iq_samples_report const *report)
{
	/*
		CTE[0]: samples type: 8 bits int, samples count 45, cte type AOA, slot durations: 2 [us], packet status CRC OK, RSSI -790
		IQ[0]: -36, 39
	*/
	printk("Starting some calculations...\n");

    size_t total_len = report->sample_count;/* number of samples, > EST_AOA_N_REF */

	double iq_i[report->sample_count];
	double iq_q[report->sample_count];
	if (report->sample_type == BT_DF_IQ_SAMPLE_8_BITS_INT) {
		for (uint8_t idx = 0; idx < report->sample_count; idx++) {
			iq_i[idx] = (double)report->sample[idx].i;
			iq_q[idx] = (double)report->sample[idx].q;
		}
	}
    size_t   n_pairs;
    double   aoa_mean, cfo_hz;
    double  *aoas = malloc(((total_len - EST_AOA_N_REF)/2) * sizeof(*aoas));
    if (!aoas) { /* handle allocation error */ }

    int err = estimate_aoa(
        iq_i, iq_q, total_len,
        aoas, &n_pairs,
        &aoa_mean, &cfo_hz
    );
    ...
    if (err != EST_AOA_OK) {
		printf("(283) estimate_aoa failed (error %d)\n", err);
	}

	/* Smooth the mean AoA */
	double aoa_filt = kalman1d_update(&kf, aoa_mean);
	printf("Computed %zu AoAs; mean = %d°, CFO = %d Hz\n",
		   n_pairs, (int)aoa_mean, (int)cfo_hz);
	/* … use aoas[0..n_pairs-1] … */
	// printk("AOA estimates: %zu, mean AOA: %d, CFO: %d Hz\n", n_pairs, (int)aoa_mean, (int)cfo_hz);
...
static void create_sync(void)
{
	...
	if (err != 0) {
		printk("(293) bt_le_per_adv_sync_create > failed (err %d)\n", err);
		return;
	}
	printk("success.\n");
}
...
static int delete_sync(void)
{
	...
	if (err != 0) {
		printk("(306) bt_le_per_adv_sync_delete > failed (err %d)\n", err);
		return err;
	}
	printk("success\n");
...
static void enable_cte_rx(void)
{
	...
	const struct bt_df_per_adv_sync_cte_rx_param cte_rx_params = {
		.max_cte_count  = 0,    /* 0 = continuous, don’t stop */
	...
	if (err != 0) {
		printk("(333) bt_df_per_adv_sync_cte_rx_enable > failed (err %d)\n", err);
		return;
	}
...
static int scan_enable(void)
{
	...
	if (!scan_enabled) {
		...
		if (err != 0) {
			printk("(366) bt_le_scan_start > failed (err %d)\n", err);
			return err;
		}
		printk("success\n");
		scan_enabled = true;
...
static void scan_disable(void)
{
	...
	err = bt_le_scan_stop();
	if (err != 0) {
		printk("(383) bt_le_scan_stop > failed (err %d)\n", err);
		return;
	}
	printk("Success.\n");
...
int main(void)
{
	...
	err = bt_enable(NULL);
	if (err != 0) {
		printk("(400) bt_enable > failed (err %d)\n", err);
	}
	printk("success\n");
	...
	do {
		...
		if (err != 0) {
			printk("(414) k_sem_take > failed (err %d)\n", err);
			return 0;
		}
		...
		if (err != 0 || sync_terminated) {
			if (err != 0) {
				printk("(428) k_sem_take > failed (err %d)\n", err);
			} else {
				printk("terminated\n");
			}
		...
		if (err != 0) {
			printk("(453) k_sem_take > failed (err %d)\n", err);
			return 0;
		}
